...
export default function SignUpPage() {
  ...
  function detectLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      err => {
        if (err.code === 1) setGpsError('Permission denied. Allow location in browser settings and try again.')
        else if (err.code === 2) setGpsError('Location unavailable. Make sure GPS is on and try again.')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!signUpData.session) { setStep('check_email'); setLoading(false); return }
  ...
  if (step === 'check_email') {
    return (
      <>
        ...
        <Link href="/sign-in" className="block w-full text-center rounded-full py-3 text-sm font-bold transition hover:opacity-90 mb-3" style={{ background: '#1d9bf0', color: 'white' }}>
          Go to sign in
        </Link>
        <button onClick={resendEmail} disabled={loading} className="w-full flex items-center justify-center gap-2 rounded-full py-3 text-sm font-semibold transition hover:opacity-80 disabled:opacity-50" style={{ border: '1px solid var(--border)', color: 'var(--text-2)', background: 'transparent' }}>
  ...
  return (
    <>
      ...
      <p className="text-sm mb-6" style={{ color: 'var(--text-2)' }}>
        {step === 'account' ? 'Connect with people in your neighborhood' : 'We use your location to show posts within 30 km'}
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {step === 'account' ? (
          <>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} required style={inputStyle} placeholder="Jane Smith" />
              </div>
              <div>
                <label className="block text-xs font-semibold mb-1.5" style={{ color: 'var(--text-3)' }}>USERNAME</label>
                <input type="text" value={username} onChange={e => setUsername(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))} required minLength={3} style={inputStyle} placeholder="janesmith" />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} required minLength={8} autoComplete="new-password" style={inputStyle} placeholder="Min 8 characters" />
            </div>
          </>
        ) : (
          <div className="space-y-4">
            ...
            {gpsError && <p className="text-xs text-center px-2" style={{ color: '#f87171' }}>{gpsError}</p>}
            {coords && <p className="text-xs text-center" style={{ color: 'var(--text-3)' }}>✓ GPS saved. You&apos;ll see posts within 30 km.</p>}
            {!coords && !detecting && !gpsError && <p className="text-xs text-center" style={{ color: 'var(--text-3)' }}>You can also set this later in profile settings.</p>}
          </div>
        )}

        {error && (
          <p className="text-sm rounded-2xl px-4 py-2.5" style={{ background: 'rgba(239,68,68,0.1)', color: '#f87171' }}>{error}</p>
        )}

        <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 rounded-full py-3 text-sm font-bold text-white transition hover:opacity-90 disabled:opacity-50" style={{ background: '#1d9bf0' }}>
          {loading && <Loader2 size={16} className="animate-spin" />}
          {step === 'account' ? 'Continue →' : 'Join Neighborhood'}
        </button>
      </form>

      {/* Google — only on account step, below form */}
      {step === 'account' && (
        <>
          ...
          <p className="text-center text-sm mt-5" style={{ color: 'var(--text-2)' }}>
            ...
            <Link href="/sign-in" className="font-bold hover:underline" style={{ color: '#1d9bf0' }}>Sign in</Link>
          </p>
        </>
      )}

      {step === 'location' && (
        <button type="button" onClick={() => setStep('account')} className="w-full text-center text-sm mt-4 transition hover:opacity-80" style={{ color: 'var(--text-3)' }}>
          ← Back
        </button>
      )}
    </>
